 ...
 * EC 注解处理器
 *
 * <p>单轮完成：先在内存里做完全部校验，全部通过才一次性生成；校验期间不写文件，不会落半成品。
 *
 * <p>生成物：
 *
 * <ul>
 *   <li><b>实体类</b>（{@code caliniya.vergvoke.base.ecs.<name>Entity}）：继承 {@code
 *       caliniya.vergvoke.base.game.Entity}，字段为各组件的扁平字段；按 {@code @Component(proc=...)} 分两档注入
 *       {@code @Updata}：
 *       <ul>
 *         <li><b>proc = "main"（或留空）</b>：轻量更新（移动、生产…），方法体<b>直接铺进实体的 {@code update()}</b>，顺序由
 *             {@code @Component.index} 决定（该档必须提供非 0 的 index）；
 *         <li><b>proc = 其他系统名</b>：需要"同阶段一致"或较重的更新（开火、寻路、AI…）， 生成独立方法 {@code
 *             update_<组件简单名>()}，由该系统在自己阶段调用。
 *       </ul>
 *   <li><b>系统分发类</b>（{@code caliniya.vergvoke.base.ecs.ECUpdates}）： {@code update_<系统名>(<实体> e)}，按
 *       {@code @Component.index} 顺序调用属于该系统的组件更新。
 *   <li><b>序列化</b>：实体按 {@code @Component.index} 顺序生成 {@code write(Writes)} / {@code read(Reads)}，
 *       各组件的读写<b>直接铺进这两个方法</b>（不再生成中间方法）：组件内部按 {@code @Save.index} 写字段； 标了 {@code @Write} /
 *       {@code @Read} 的组件则把方法体原样复制过来，外面套一层 {@code { }} 包装 （参数名不同就在包装里起个别名；方法体里有 {@code return}
...
public class ECProcessor extends Processor {
  ...
  private boolean validateSaveTargets(Map<String, AType> components) {
    ...
    for (AVar field : fields(Save.class)) {
      ...
      Map<Integer, String> indexes =
          byComponent.computeIfAbsent(owner.fullName(), key -> new LinkedHashMap<>());
  ...
  private boolean validateSerializationMethods(Map<String, AType> components) {
    ...
    valid &=
        checkSerializationMethods(methods(Write.class), components, WRITES_CLASS, "@Write", writes);
    valid &=
        checkSerializationMethods(methods(Read.class), components, READS_CLASS, "@Read", reads);
   ...
   *
   * <p>序列化方法的方法体会被内联进实体的 {@code write()} / {@code read()}，那里的 return 会直接跳过后面所有组件，
  ...
  /**
   * 生成实体的 {@code write(Writes)} / {@code read(Reads)}：按 @Component.index 顺序，把各组件要写/读的东西
   * <b>直接铺进方法体</b>（不生成 write_&lt;组件&gt; / read_&lt;组件&gt; 这种中间方法）。
   *
   * <ul>
   *   <li>{@code @Save} 字段 → 直接展开成 {@code w.i(x)} / {@code x = r.i()}；
   *   <li>{@code @Write} / {@code @Read} → 方法体原样复制过来，外面套一层花括号当作用域；参数名和实体这边不一样时， 先来一句 {@code Writes
  ...
  private MethodSpec serializationOverride(boolean writing, EntityPlan plan, boolean callSuper) {
    ...
    for (SerialPiece piece : plan.serials) {
      if (piece.methodStyle()) {
        ...
        String alias =
            param.equals(source) ? null : ioClass.simpleName() + " " + source + " = " + param + ";";
   ...
   * }</pre>
   *
   * <p>方法体里写了 {@code return} 时，包装里还会多一层守卫：正常走完才算完成，中途 return 出去就直接抛异常 （内联之后 return
  ...
  private void addAbstractStubs(
    ...
    for (Element member : elementUtils.getAllMembers(base)) {
      ...
      StringBuilder signature =
          new StringBuilder(name).append('/').append(method.getParameters().size());
