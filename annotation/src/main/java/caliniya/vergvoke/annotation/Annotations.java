...
public class Annotations {
    ...
    public @interface Entity {
        ...
        /**
         * 该实体对应的类型目标类（必须实现 {@code caliniya.vergvoke.base.api.EntityType}）。
         *
         * <p>例：{@code @Entity(comps = ..., name = "Unit", type = UnitType.class)}
         */
        Class<?> type();
    }
     ...
     * 组件方法覆写实体基类的同名同参方法。
     *
     * <p>生成实体时该方法体铺进实体类，并打上 {@code @Override}，替代基类默认实现。
     * 签名必须与 {@code Entity} 上已有方法一致；同一实体内同签名只能有一个组件覆写。
     */
    @Retention(RetentionPolicy.SOURCE)
    @Target({ ElementType.METHOD })
