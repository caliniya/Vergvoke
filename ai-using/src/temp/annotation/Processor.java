...
public abstract class Processor extends AbstractProcessor {
    ...
    /**
     * 读取 {@code @Entity.type()} 的类型镜像。
     *
     * <p>注解里的 {@code Class} 值在编译期用 {@code getAnnotation().type()} 会抛
     * {@code MirroredTypeException}，只能从 AnnotationMirror 取 TypeMirror。
     *
     * @return 类型目标；未声明或不是 Class 时返回 null
     */
    protected TypeMirror entityTypeOf(AType a) {
        for (AnnotationMirror am : a.mirror) {
            if (!am.getAnnotationType().toString()
                    .equals("caliniya.vergvoke.annotation.Annotations.Entity")) {
                continue;
            ...
            for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> e : am.getElementValues()
                    .entrySet()) {
                if (!e.getKey().getSimpleName().contentEquals("type"))
                    continue;
                Object v = e.getValue().getValue();
